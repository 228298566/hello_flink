...
object CoProcessFunctionDemo {
  def main(args: Array[String]): Unit = {
    ...
    readings
      ...
      .process(new CoProcessFunction1)
      .print()

    env.execute()
  }

  class CoProcessFunction1 extends CoProcessFunction[SensorReading, (String, Long), SensorReading] {
    // 初始化传送数据的开关，默认值是false
    // 只针对当前key可见的状态变量
    lazy val forwardingEnabled = getRuntimeContext.getState(new ValueStateDescriptor[Boolean]("filter-switch", Types.of[Boolean]))
